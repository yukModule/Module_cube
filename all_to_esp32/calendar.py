from putpic import show_words,show_word
...
def calshow():
    ...
    for i in k:
        ...
        x=0
        for j in i:
            show_words(j, 13+33*x,25+35*y, 255, 255, 255,0, 0, 0)
            if x==0 or x==6:
                show_words(j, 13+33*x,25+35*y, 255, 0, 0,0, 0, 0)
            if j==days:
                show_words(j, 13+33*x,25+35*y, 0, 90, 255,0, 0, 0)
            x+=1
    year=str(year)
    if month<10:
        month='0'+str(month)
    ...
    if day<10:
        ...
        day=str(day)
    show_word(year+'-'+month+'-'+day, 38,15, 255, 219, 0,0, 0, 0)
